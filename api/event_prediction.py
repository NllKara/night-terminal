...
def predict_event(event:str,credentials:dict|None=None):
 ...
 if not key:return {"ok":False,"event":e,"label":cfg["label"],"error":"FRED API key required for pre-release macro evidence","available_events":event_catalog()}
 try:target=_fred(cfg["target"],key)
 except Exception as ex:return {"ok":False,"event":e,"label":cfg["label"],"error":"Target data unavailable: "+type(ex).__name__,"available_events":event_catalog()}
 tch=_changes(target);evidence=[];score=0.0;weight_sum=0.0
 for sid,name,effect,w in cfg["drivers"]:
  try:rows=_fred(sid,key);ch=_changes(rows)
