...
app = FastAPI(title="NIGHT Quant Terminal API", version="0.7.0")
...
@app.get("/api/health")
def health():
    return {"status": "ok", "service": "night-quant-terminal", "version": "0.7.0"}
...
async def analyse_live_bars(req: LiveBarsRequest):
    ...
    result = analyse(req.symbol, req.timeframe, {"bars": req.bars[-240:], "source": req.source, "volume_type": req.volume_type, "macro": macro_data, "event_risk": event_risk})
...
def equity(req: EquityRequest):
    creds=req.credentials.model_dump(exclude_none=True) if req.credentials else {}
    return equity_snapshot(req.symbol,creds.get("twelve_key"))

@app.post("/api/luna")
def luna(req: LunaRequest):
    return ask_luna(req.message,req.context or {})
...
def quant_chat(req: ChatRequest):
    return ask_luna(req.message,req.analysis or {})
...
@app.get("/api/modules")
def modules():
    return {"modules": ["Realtime Browser Tick Stream","Realtime Tick Candle Builder","Live OHLCV Adapter","Equity Market Data","SEC EDGAR Fundamentals","Twelve Data Global Fundamentals","Exact MTF Quant","Volume/Flow","Regime","Probability/EV","FRED Macro","GDELT News Intelligence","CFTC COT Positioning","WTI/Brent/NatGas","Live AIS Shipping","Trade Readiness","Luna AI","Report Engine"]}
