...
app=FastAPI(title="NIGHT Quant Terminal API",version="0.9.0");market=MarketProvider();macro=MacroProvider()
...
def _event_risk_from_news(news:dict)->float:
    keys=["federal reserve","fed chair","pce","cpi","payroll","nfp","war","attack","tariff","hormuz","sanction","rate decision","shipping","tanker","red sea"];hits=0
    for a in (news.get("articles") or [])[:18]:
        t=(a.get("title") or "").lower();hits+=sum(1 for k in keys if k in t)
    return min(86.0,18.0+hits*4.0)
@app.get("/api/health")
def health():return {"status":"ok","service":"night-quant-terminal","version":"0.9.0"}
...
async def _institutional_context(symbol:str,creds:dict):
    ...
    if symbol.upper()=="XAUUSD":macro_data["macro_score"]=_clamp(.65*base_macro+.20*news_score+.15*cot_score);macro_data["intermarket_score"]=_clamp(.85*base_inter+.15*news_score)
    else:macro_data["macro_score"]=_clamp(.80*base_macro+.20*news_score)
    event_risk=_event_risk_from_news(news);return macro_data,news,cot,activity,event_risk,base_macro,base_inter,news_score,cot_score
async def _decorate_result(symbol,result,macro_data,news,cot,activity,event_risk,base_macro,base_inter,news_score,cot_score):
    result["macro_source"]=macro_data.get("macro_source");result["macro_details"]=macro_data.get("macro_details",{});result["news"]=news;result["cot"]=cot;result["activity"]=activity;result["event_risk"]=event_risk;result["institutional_factors"]={"fred_macro":round(base_macro,4),"fred_intermarket":round(base_inter,4),"gdelt_news":round(news_score,4),"cftc_cot":round(cot_score,4),"blended_macro":round(float(macro_data.get("macro_score",0.0)),4)};return result
async def _analyse_one(symbol,timeframe,creds):
    market_data=await market.snapshot(symbol,timeframe,creds);macro_data,news,cot,activity,event_risk,base_macro,base_inter,news_score,cot_score=await _institutional_context(symbol,creds);result=analyse(symbol,timeframe,{**market_data,"macro":macro_data,"event_risk":event_risk});result["provider_errors"]=market_data.get("provider_errors",[]);return await _decorate_result(symbol,result,macro_data,news,cot,activity,event_risk,base_macro,base_inter,news_score,cot_score)
...
async def analyse_live_bars(req:LiveBarsRequest):
    creds=req.credentials.model_dump(exclude_none=True) if req.credentials else {};macro_data,news,cot,activity,event_risk,base_macro,base_inter,news_score,cot_score=await _institutional_context(req.symbol,creds);result=analyse(req.symbol,req.timeframe,{"bars":req.bars[-240:],"source":req.source,"volume_type":req.volume_type,"macro":macro_data,"event_risk":event_risk});result["stream_realtime"]=True;return await _decorate_result(req.symbol,result,macro_data,news,cot,activity,event_risk,base_macro,base_inter,news_score,cot_score)
