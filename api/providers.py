    ...
    async def snapshot(self, symbol: str, timeframe: str, credentials: dict | None = None) -> dict:
        ...
        if token and symbol.upper() in {"XAUUSD","EURUSD","GBPUSD","USDJPY"}:
            try:
                ...
                if len(bars)>=30:return {"bars":bars,"source":"OANDA practice v20","volume_type":"tick volume (price-update count)","freshness":1.0}
        ...
        if bsym:
            try:
                ...
                if len(bars)>=30:return {"bars":bars,"source":"Binance public market data","volume_type":"exchange traded base-asset volume","freshness":1.0}
        ...
        if td_key:
            try:
                ...
                if len(bars)>=30:
                    ...
                    return {"bars":bars,"source":"Twelve Data Basic/REST","volume_type":vt,"freshness":0.95}
...
class MacroProvider:
    ...
    async def snapshot(self, symbol: str, credentials: dict | None = None) -> dict:
        ...
        try:
            ...
            # USD/gold macro impulse: rising real/nominal yields favor USD and pressure gold; rising unemployment is dovish; accelerating CPI supports gold inflation hedge but can also lift yields, so smaller weight.
